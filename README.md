...
강의를 들으며 기록한 음성과 문서가 서로 분리되어 있으면, 나중에 전사를 읽더라도 특정 발언이 어떤 슬라이드를 가리키는지 찾기 어렵습니다. AnythingNote는 강의 발언, 타임스탬프, 당시의 PDF 페이지, 중요도를 하나의 Memory Unit으로 연결해 이 문제를 해결합니다.
...
- **문상혁** · 응용수학과 / 컴퓨터공학과 · 전체 파이프라인 총괄
- **현효준** · 응용수학과 · 시연 및 PPT 제작
- **장건오** · 응용수학과 · 음성 처리 모듈 구현
- **정진호** · 응용수학과 · 비전 처리 모듈 구현
