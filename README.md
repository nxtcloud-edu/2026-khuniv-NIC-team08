...
| 🎓 **실제 강의** | `npm run dev:example` | `http://localhost:5173/?demo=example` | 실제 ML 강의 52분 · PDF 68페이지 · 전사 495구간 · 대표 4페이지 · **영상 연동** |

> 🎬 **시연 영상 녹화는 `npm run dev:example`** 로 실행할 것. 영상 연동은 실제 강의 데모에만 있음.
...
fixture 무결성(PDF 68페이지 · 전사 495구간 · 페이지 이미지 · 영상) → 전체 테스트 → 타입 검사와 프로덕션 빌드 순으로 점검함.
...
flowchart LR
    subgraph P1["① 데이터 준비 (사전 처리)"]
        ...
        A2[녹음 전사] --> H
        A3[강의 영상] --> H
        H --> B["Memory Unit<br/>example/session.json"]
...
자동 변환과 수기 선별로 나뉨. 현재 MVP는 원본에서 `Memory Unit`을 자동 생성하지 않음.

**자동** — `npm run example:prepare` (유지보수자 전용, `ffmpeg` 필요)

1. 📄 이미지 기반 PDF에서 대표 페이지 JPEG 추출.
2. 🗣️ 전사의 벽시계 시간을 **영상 시작 시각 기준 상대 시간**으로 환산.
3. 🎬 영상의 참가자 영역 제거 + 해상도·프레임률을 낮춘 **공개용 MP4** 생성.

**수기** — `example/session.json`

4. 🔗 495개 전사 구간 중 의미 있는 순간을 **사람이 선별**해 *발언 ↔ 페이지 ↔ 중요도*로 묶어 `Memory Unit` 작성. (실제 강의 세션 기준 4개)

**검증** — `npm run example:check`

5. ✅ 작성된 `Memory Unit`이 전사 원문·타임스탬프·PDF 페이지와 일치하는지 대조. 즉 생성기가 아니라 **검증기**임.

> 원본 → `Memory Unit` 자동 생성은 다음 단계 과제.
