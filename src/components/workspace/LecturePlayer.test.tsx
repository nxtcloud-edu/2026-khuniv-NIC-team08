...
describe("LecturePlayer", () => {
  ...
  it("선택된 발언의 시각으로 이동한다", () => {
    const { rerender } = render(
      <LecturePlayer src="/example/lecture.mp4" seek={{ seconds: 40 }} />,
    );
    const video = screen.getByLabelText("강의 영상 플레이어") as HTMLVideoElement;
    expect(video.currentTime).toBe(40);

    rerender(<LecturePlayer src="/example/lecture.mp4" seek={{ seconds: 535 }} />);
  ...
  it("같은 시각을 다시 요청해도 다시 이동한다", () => {
    ...
    rerender(<LecturePlayer src="/example/lecture.mp4" seek={{ seconds: 40 }} />);
